...
- 进入地点前的道路滚动过渡，采用等比放大后的轻微平移与学生行走进度插画，支持“减少界面动效”设置
...
- 鼠标停在地图建筑上会显示单一白色纸质活动浮签，直接说明该地点可以进行的两项行动；点击后再固定路线与完整地点介绍
...
- 完整七天通常包含 17 次耗时核心选择：固定日程推进约 2 个时段，自主地点或人物行动推进约 3 个时段，固定事件不会被跨过
- 阶段承诺与睡眠债、技术债、人情债、AI 风险；承诺落空和取巧会在之后真实追账
- 数据驱动事件、即时与延迟后果；回执会明确指出后果来自哪一天的哪项选择
- 考试和展示综合准备、状态、关系、核验证据与既往债务判定质量，优质结局不能只靠堆高单项数值获得
